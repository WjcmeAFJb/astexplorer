...
CodeMirror.defineMode('clike', function(config: any, parserConfig: any) {
  ...
  function tokenBase(stream: CMStream, state: CMState) {
    var ch = stream.next();
    if (hooks[ch]) {
  ...
  function tokenComment(stream: CMStream, state: CMState) {
  ...
  function Context(indented: number, column: number, type: string, align: boolean | null, prev: CMContext | null) {
  ...
  function pushContext(state: CMState, col: number, type: string) {
  ...
  function popContext(state: CMState) {
...
function cppHook(stream: CMStream, state: CMState) {
...
function def(mimes: string | string[], mode: CMMode) {
  ...
  if (words.length) {
    ...
    CodeMirror.registerHelper('hintWords', mimes[0], words as string[]);
  }
