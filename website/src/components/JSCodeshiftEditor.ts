  ...
  componentDidMount() {
    ...
    this.codeMirror.setOption('extraKeys', {
      'Ctrl-Space': (cm: CodeMirror.Editor) => {
        if (server !== undefined) server.complete(cm);
      },
      'Ctrl-I': (cm: CodeMirror.Editor) => {
        if (server !== undefined) server.showType(cm);
      },
      'Ctrl-O': (cm: CodeMirror.Editor) => {
        if (server !== undefined) server.showDocs(cm);
      },
    });
...
interface TernServerScope {
  props: Record<
    string,
    { getFunctionType?: () => { propagate: (callee: unknown) => void } } | undefined
  >;
}
...
function loadTern(): void {
  require(['codemirror/addon/hint/show-hint', 'codemirror/addon/tern/tern', 'acorn'], (
    _showHint: unknown,
    _ternAddon: unknown,
          ...
          if (fnVal !== undefined && fnVal !== null) {
            ...
            if (fnType !== undefined) {
              ...
              fnType.propagate(
                new infer.IsCallee(
                  infer.cx().topScope,
                  [cx.definitions.jscodeshift.file, cx.definitions.jscodeshift.apiObject],
                  null,
                  infer.ANull,
                ),
              );
