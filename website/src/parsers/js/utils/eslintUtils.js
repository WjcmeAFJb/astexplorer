...
export function formatResults(/** @type {Record<string, unknown>} */ results) {
  return results.length === 0
    ? 'Lint rule not fired.'
    : results.map(formatResult).join('').trim();
}

export function formatResult(/** @type {Record<string, unknown>} */ result) {
...
export function defineRule(/** @type {Record<string, unknown>} */ eslintRules, /** @type {string} */ code) {
...
export function runRule(/** @type {string} */ code, /** @type {Record<string, unknown>} */ eslint, /** @type {Record<string, unknown>} */ sourceCode) {
