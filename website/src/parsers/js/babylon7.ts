...
export const parserSettingsConfiguration = {
  fields: [
    ...
    },
    {
      ...
      settings: (settings: Record<string, unknown>) => settings.pipelineOptions || defaultOptions.pipelineOptions,
    },
    {
      ...
      settings: (settings: Record<string, unknown>) => settings.decoratorOptions || defaultOptions.decoratorOptions,
    },
    {
      ...
      settings: (settings: Record<string, unknown>) => settings.typescriptOptions || defaultOptions.typescriptOptions,
    }
  ],
};
...
export default {
  ...
  loadParser(callback: (realParser: typeof import('babylon7')) => void) {
    require(['babylon7'], callback);
  },

  parse(babylon: typeof import('babylon7'), code: string, options: Record<string, unknown>) {
    options = {...options};
