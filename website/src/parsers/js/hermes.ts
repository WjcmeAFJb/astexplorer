...
export default {
  ...
  loadParser(callback: (realParser: HermesWorkerClient) => void) {
    callback(new HermesWorkerClient());
  },

  async parse(hermes: HermesWorkerClient, code: string, options: Record<string, unknown>) {
    return await hermes.parse(code, options);
  },

  nodeToRange(node: ESTreeNode) {
    // For `babel: true` mode
